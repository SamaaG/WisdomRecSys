...
object SparkLDAMain {
  ...
  private def run(params: Params) {
    ...
    val test_input = Seq("I'm so over T-Bones.I used to relish the thought of hanging out on their big deck on one of the first warm days of the year, treating myself to their yummy grilled chicken salad or wings--the only 2 things on their menu that I liked.\n  I was even willing to tolerate the funky smell of the main restaurant on my way out to the deck.\n  But the last 3 times I've been to T-Bones, the food and service have been awful.  \nAnd they changed the menu, removing the grilled chicken breast salad that I liked.  \nThe sweet tea was almost as clear as water, and painfully sweet--undrinkable.I'm sorry, T-Bones.  \nWe tried to make it work, but it takes two.  Please stop relying on the fact that you're the only place (I know of) in the area where you can eat the water's edge.");
    ...
    val test_document: RDD[(Long, Vector)] = test(sc);
    ...
    //topicDistributions.collect().foreach(f=>println(f.toString()))
  ...
  private def test(sc: SparkContext): (RDD[(Long, Vector)]) = {
    ...
    val df = sc.textFile ("/home/poojashekhar/Downloads/CS5560 - Tutorial 9 Source Code/Spark_KMeans_FV/data/20_news_group/Test_data/testdoc" ).map (f => CoreNLP.returnLemma (f) ).filter (_.length > 3).toDF ("docs")
